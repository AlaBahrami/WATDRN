...
subroutine read_parameters_nc( &
    ...
    character(len = DEFAULT_FIELD_LENGTH) :: &
        dim_x = '', dim_y = '', dim_m = '', dim_c = '', dim_v = '', dim_s = '', projection, units, field, code, dim_names
    ...
    if (ierr /= 0) then
        ...
        call nc4_get_dimension_name(iun, 1, dim_name = dim_names, ierr = ierr)
        if (ierr /= 0) goto 999
    !> Check bounds.		
    ...
    if (lowercase(dim_names) == 'subbasin') then
    ...
    do i = 1, nvars
        ...
        select case (lowercase(field))
            ...
            case ('fcan')
			    ...
			    if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_c, pm%grid%fcan, fill_r, ierr = ierr)
				else				
                    if (ndims == 2) then
            ...
            case ('lnz0')
                if (RUNCLASS36_flgs%PROCESS_ACTIVE .or. svs_mesh%PROCESS_ACTIVE) then
			    if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_c, pm%grid%lnz0, fill_r, ierr = ierr)
                else
                    if (ndims == 2) then
                        call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                        do n = 1, shd%NA
            ...
            case ('vf')
			    ...
			    if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_v, dat2_r, fill_r, ierr = ierr)
                    do m = 1, nvf
                        do k = 1, shd%lc%NML
            ...
            case ('sdep')
                if (RUNCLASS36_flgs%PROCESS_ACTIVE .or. svs_mesh%PROCESS_ACTIVE) then
			      if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, pm%grid%sdep, fill_r, ierr = ierr)
				  else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('xslp')
                if (RUNCLASS36_flgs%PROCESS_ACTIVE .or. svs_mesh%PROCESS_ACTIVE) then
			      if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, pm%grid%xslp, fill_r, ierr = ierr)
				  else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('dd', 'dden')
                if (RUNCLASS36_flgs%PROCESS_ACTIVE .or. svs_mesh%PROCESS_ACTIVE) then
			      if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, pm%grid%dd, fill_r, ierr = ierr)
				  else
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)	
                    do n = 1, shd%NA
            ...
            case ('sand')
                if (RUNCLASS36_flgs%PROCESS_ACTIVE .or. svs_mesh%PROCESS_ACTIVE) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_s, pm%grid%sand, fill_r, ierr = ierr)
				else				
                    if (ndims == 2) then
                        call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                        do n = 1, shd%NA
            ...
            case ('clay')
                if (RUNCLASS36_flgs%PROCESS_ACTIVE .or. svs_mesh%PROCESS_ACTIVE) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_s, pm%grid%clay, fill_r, ierr = ierr)
				else				
                    if (ndims == 2) then
                        call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                        do n = 1, shd%NA
            ...
            case ('orgm')
                if (RUNCLASS36_flgs%PROCESS_ACTIVE .or. svs_mesh%PROCESS_ACTIVE) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_s, pm%grid%orgm, fill_r, ierr = ierr)
				else				
                    if (ndims == 2) then
                        call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                        do n = 1, shd%NA
            ...
            case ('iwf')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, pm%grid%iwf, fill_i, ierr = ierr)
				else
            ...
            case ('pwr')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, bflm%pm_grid%pwr, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('flz')
                if (bflm%BASEFLOWFLAG == 2) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, bflm%pm_grid%flz, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('r1n')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, rtepm%r1n, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('r2n')
                if (rteflg%PROCESS_ACTIVE) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, rtepm%r2n, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('mndr')
                if (rteflg%PROCESS_ACTIVE) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, rtepm%mndr, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('widep')
                if (rteflg%PROCESS_ACTIVE) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, rtepm%widep, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('aa2')
                if (rteflg%PROCESS_ACTIVE) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, rtepm%aa2, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('aa3')
                if (rteflg%PROCESS_ACTIVE) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, rtepm%aa3, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('aa4')
                if (rteflg%PROCESS_ACTIVE) then
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, rtepm%aa4, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
...
!                case ('kcond')
            ...
            case ('fetch')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, pbsm%pm_grid%fetch, fill_r, ierr = ierr)
				else
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('ht')
                if (pbsm%PROCESS_ACTIVE) then
			    if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, pbsm%pm_grid%Ht, fill_r, ierr = ierr)
				else
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('n_s')
                if (pbsm%PROCESS_ACTIVE) then
			    if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, pbsm%pm_grid%N_S, fill_r, ierr = ierr)
				else
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('a_s')
                if (pbsm%PROCESS_ACTIVE) then
			    if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, pbsm%pm_grid%A_S, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('distrib')
                if (pbsm%PROCESS_ACTIVE) then
			    if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, pbsm%pm_grid%Distrib, fill_r, ierr = ierr)
				else				
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                    do n = 1, shd%NA
            ...
            case ('elevation')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_m, mountain_mesh%pm%elev, fill_r, ierr = ierr)
				else
                    if (ndims == 2) then
                      call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                      do n = 1, shd%NA
            ...
            case ('slope')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_m, mountain_mesh%pm%slope, fill_r, ierr = ierr)
				else				
                    if (ndims == 2) then
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                      do n = 1, shd%NA
            ...
            case ('aspect')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_m, mountain_mesh%pm%aspect, fill_r, ierr = ierr)
				else
                    if (ndims == 2) then
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                      do n = 1, shd%NA
            ...
            case ('delta')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_m, mountain_mesh%pm%delta, fill_r, ierr = ierr)
				else				
                    if (ndims == 2) then
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                       do n = 1, shd%NA
            ...
            case ('delta_elevmax')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_m, mountain_mesh%pm%delta_elevmax, fill_r, ierr = ierr)
				else				
                    if (ndims == 2) then
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                       do n = 1, shd%NA
            ...
            case ('curvature')
				...
				if (lowercase(dim_names) == 'subbasin') then
                    call nc4_get_variable(iun, field, dim_x, dim_m, mountain_mesh%pm%curvature, fill_r, ierr = ierr)
				else				
                    if (ndims == 2) then
                    call nc4_get_variable(iun, field, dim_x, dim_y, dat2_r, fill_r, ierr = ierr)
                       do n = 1, shd%NA
